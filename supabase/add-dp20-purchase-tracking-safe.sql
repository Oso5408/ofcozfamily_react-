-- =============================================
-- DP20 Package Purchase Tracking System (SAFE VERSION)
-- =============================================
-- Safely creates/updates package_purchases table and policies
-- Can be run multiple times without errors
-- =============================================

-- Create package_purchases table (if not exists)
...
DROP POLICY IF EXISTS "Users can create own purchases" ON public.package_purchases;
DROP POLICY IF EXISTS "Admins can view all purchases" ON public.package_purchases;
DROP POLICY IF EXISTS "Admins can update all purchases" ON public.package_purchases;
...
DROP TRIGGER IF EXISTS set_package_purchases_updated_at ON public.package_purchases;
DROP FUNCTION IF EXISTS update_package_purchases_updated_at();

-- Create function
CREATE FUNCTION update_package_purchases_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
...
CREATE FUNCTION reject_purchase(
  purchase_id UUID,
  admin_id UUID,
...
-- Check table exists
SELECT
  '✅ package_purchases table exists' AS status,
...
-- Check policies
SELECT
  '✅ RLS policies active (' || COUNT(*)::TEXT || ')' AS status
FROM pg_policies
WHERE tablename = 'package_purchases';

SELECT '🎉 DP20 Purchase Tracking System ready!' AS status;
